...
let globalX = 0;
let globalY = 0;
let moveX=0,moveY=0,move=false,moveShape=null;
let init_x,init_y,init_px,init_py;
let data;
let shape;
let hold = false;
let cnv;
...
function lineIntersectLine(line1,line2) {
  ...
  if(x>0 && y>0){
        for(var i=0;i<arrayOfShapes.length;i++){
          if(arrayOfShapes[i].tool==="Ellipse"){
            var r=((arrayOfShapes[i].px - arrayOfShapes[i].x)*(arrayOfShapes[i].px - arrayOfShapes[i].x)) +((arrayOfShapes[i].py - arrayOfShapes[i].y)*(arrayOfShapes[i].py - arrayOfShapes[i].y));
            if(doesCircleInterceptCircle(x,y,arrayOfShapes[i].x,arrayOfShapes[i].y,0.5,Math.sqrt(r))){
              return i;
            }
          }
...
function poitsOfIntersection(arrayOfShapes,i){
  var points=[]
  if(arrayOfShapes[i].tool=="Line"){
    for(var t=0;t<arrayOfShapes.length;t++){
      if(t!==i){
        if(arrayOfShapes[t].tool==="Line"){
          if(lineIntersectLine(arrayOfShapes[t],arrayOfShapes[i])){
            points.push(lineIntersectLine(arrayOfShapes[t],arrayOfShapes[i]));
          }
        }
        else if(arrayOfShapes[t].tool==="Ellipse"){
...
function trimIt(arrayOfShapes,i,x,y){
var intersectionPoints=poitsOfIntersection(arrayOfShapes,i);
}
...
function Canvas2() {
  ...
  const mouseDragged = (p5) => {
    ...
    if(hold===false && move===true){
      p5.background(p5.color(bgColor));
      let deltaX=p5.mouseX-moveX,deltaY=p5.mouseY-moveY
      if(moveShape!==null){
      arrayOfShapes[moveShape].x=init_x+deltaX
      arrayOfShapes[moveShape].y=init_y+deltaY
      arrayOfShapes[moveShape].px=init_px+deltaX
      arrayOfShapes[moveShape].py=init_py+deltaY
      }
      else{
        move=false
        moveShape=null
  ...
  if(hold===false && shape==="Delete"){
    ...
    if(hold===false && shape==="Move"){
      moveX=p5.mouseX
      moveY=p5.mouseY
      moveShape=intersectionIndex(arrayOfShapes,moveX,moveY);
      if(moveShape!==null){
        init_x=arrayOfShapes[moveShape].x;
        init_y=arrayOfShapes[moveShape].y;
        init_px=arrayOfShapes[moveShape].px;
        init_py=arrayOfShapes[moveShape].py;
      }
      else{
        move=false
        moveShape=null
  ...
  console.log(shape)
  if(hold===false && shape!=="Delete" && shape!=="" && shape!=="Move"){
      hold=true
  }
  else{
    hold = false;
  }
  if(hold===false && shape==="Move" && move===false){
    move=true
  }
  else {
    move=false
